...
- `function` や `#'` によって得られる関数オブジェクトの表示において、これまでアドレスしか出せなかったが、関数名も表示できるようにした
...
- `(defclass)` で、同じスーパークラスが複数経路から継承される（菱形継承）場合はエラーにするようにした
...
- 空の配列が `#()` ではなく、`#)` と表示されていた不具合を修正
- 実行モジュールに、最後に評価した値を表示するオプション `-p` を追加
