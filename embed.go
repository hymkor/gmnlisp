...
var embedFunctions = map[Symbol]Node{
	...
	NewSymbol("setf"):        &LispString{S: `(let ((setf-table '((car . set-car) (cdr . set-cdr) (elt . set-elt) (dynamic . set-dynamic) (subseq . set-subseq) (setq . set-setq) (assoc . set-assoc) (aref . set-aref)))) (lambda-macro (expr newvalue) (if (symbolp expr) (list 'setq expr newvalue) (let* ((name (car expr)) (pair (assoc name setf-table)) (tmp nil) (setter (if pair (cdr pair) (progn (setq tmp (convert (string-append "set-" (convert name <string>)) <symbol>)) (setq setf-table (cons (cons name tmp) setf-table)) tmp))) (arguments (cdr expr))) (cons setter (cons newvalue arguments))))))`},
	NewSymbol("set-elt"):     &LispString{S: `(lambda-macro (newvalue seq &rest z) (list 'if (list 'arrayp seq) (list 'set-aref newvalue seq @z) (list 'setf seq (list 'swap-elt newvalue seq @z))))`},
