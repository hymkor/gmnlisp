...
var embed_swap_elt = &LispString{S: `(lambda (source z newvalue) (if (stringp source) (string-append (subseq source 0 z) (create-string 1 newvalue) (subseq source (1+ z) (length source))) (let ((s source)) (while s (if (zerop z) (set-car newvalue s)) (setq z (1- z)) (setq s (cdr s))) source)))`}

// NewSymbol("swap-subseq"): embed_swap_subseq,
var embed_swap_subseq = &LispString{S: `(lambda (seq start end newvalue) (if (stringp seq) (string-append (subseq seq 0 start) newvalue (subseq seq end (length seq))) (let ((orig seq)) (while seq (if (and (<= start 0) (> end 0) newvalue) (progn (set-car (car newvalue) seq) (setq newvalue (cdr newvalue)))) (setq start (1- start)) (setq end (1- end)) (setq seq (cdr seq))) orig)))`}
