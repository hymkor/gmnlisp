...
var embedFunctions = map[Symbol]Node{
	NewSymbol("incf"):        &LispString{S: `(lambda-macro (s) (list 'setq s (list '+ s '1)))`},
	NewSymbol("decf"):        &LispString{S: `(lambda-macro (s) (list 'setq s (list '- s '1)))`},
	...
	NewSymbol("set-assoc"):   &LispString{S: `(lambda-macro (newvalue key m) (list 'let* (list (list 'L m) (list 'K key) (list 'tmp (quote nil))) (list 'while 'L (list 'if (list 'and (list 'setq 'tmp (list 'car 'L)) (list 'consp 'tmp) (list 'equal 'K (list 'car 'tmp))) (list 'set-car newvalue 'L)) (list 'setq 'L (list 'cdr 'L)))))`},
}
